...
export class MapsProvider {
  ...
  public setSelection(shape) {
    if (shape.type !== 'marker') {
      MapsProvider.prototype.clearSelection();
      shape.setEditable(true);
      MapsProvider.prototype.selectColor(shape.get('fillColor') || shape.get('strokeColor'));
    }
  ...
  makeColorButton(color, google) {
    ...
    google.maps.event.addDomListener(button, 'click', function () {
       MapsProvider.prototype.selectColor(color);
       MapsProvider.prototype.setSelectedShapeColor(color,google);
    });
  ...
  buildColorPalette(google) {
    ...
    for (var i = 0; i < colors.length; ++i) {
     ...
     MapsProvider.prototype.selectColor(colors[0]);
  }
  ...
  createDrawingTool(map, google) {
    ...
    google.maps.event.addListener(drawingManager, 'overlaycomplete', function (e) {
      ...
      if (e.type !== google.maps.drawing.OverlayType.MARKER) {
        ...
        google.maps.event.addListener(newShape, 'click', function (e) {
          ...
          MapsProvider.prototype.setSelection(newShape);
        });
        MapsProvider.prototype.clearSelection();
      }
      else {
    ...
    // map is clicked.
    google.maps.event.addListener(drawingManager, 'drawingmode_changed', MapsProvider.prototype.clearSelection);
    google.maps.event.addListener(map, 'click', MapsProvider.prototype.clearSelection);
    google.maps.event.addDomListener(document.getElementById('delete-button'), 'click', MapsProvider.prototype.deleteSelectedShape);
    MapsProvider.prototype.buildColorPalette(google);
  ...
  calcRoute(start, end, google, map,showPeajes,showGrifos) {
    ...
    directionsService.route(request, function (response, status) {
      ...
      if (status == google.maps.DirectionsStatus.OK) {
         MapsProvider.prototype.clearMarkers();
          if(cityCircle){
      ...
      radius:  1000
          ...
          MapsProvider.prototype.buscarGrifos(centro,google,map);
      ...
      cityCircle.addListener('mouseout', function () {
        ...
        for (var i = response.routes.length - 1; i >= 0; i--) {
          ...
          var line = MapsProvider.prototype.drawPolyline(response.routes[i], color, google, map)
          polylines.push(line);
          ...
          google.maps.event.addListener(line, 'click', function (event) {
            ...
            console.log("index=", index);
             MapsProvider.prototype.highlightRoute(index, google, map);
          });
  ...
  currentPos(google, map) {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        MapsProvider.prototype.handleLocationError(true, infowindow, map.getCenter(),map);
      });
    } else {
      // Browser doesn't support Geolocation
      MapsProvider.prototype.handleLocationError(false, infowindow, map.getCenter(), map);
    }
  }
  ...
  //console.log("featurePos=",featurePos);
      ...
      distancia = MapsProvider.prototype.calcDistance(pm,fm,google);
      if (distancia <= radius){
     ...
     clearMarkers() {
      MapsProvider.prototype.setMapOnAll(null);
    }

    // Shows any markers currently in the array.
     showMarkers(map) {
      MapsProvider.prototype.setMapOnAll(map);
    }
